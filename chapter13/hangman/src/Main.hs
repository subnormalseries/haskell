...
gameWords = do
 ...
 where
  gameLength w =
    let wordLength = length (w :: String)
    in  (wordLength >= minWordLength) && (wordLength <= maxWordLength)
...
freshPuzzle word = Puzzle word (map (mappingFunction) word) []

mappingFunction :: Char -> Maybe Char
mappingFunction _ = Nothing
...
gameOver :: Puzzle -> IO ()
gameOver (Puzzle wordToGuess _ guesses) = if (length guesses) > 15
  then do
    putStrLn "You lose!"
